...
x <- seq(0.75,1,by=0.001)
par(mar=c(4,5,2,2))
k=0.036
plot(x,0.75/x,type="l",lwd=2,xlim=c(k,1-k),ylim=c(k,1-k),xlab=expression('x'[1]),
...
save(S1,file="Data/S1")
save(S2,file="Data/S2")
save(S3,file="Data/S3")
save(S1_up,file="Data/S1_up")
save(S2_up,file="Data/S2_up")
save(S3_up,file="Data/S3_up")
save(S1_low,file="Data/S1_low")
save(S2_low,file="Data/S2_low")
save(S3_low,file="Data/S3_low")
...
plot(seq(n/100,n,by=n/100),S1*100,type="l",lwd=2,xlab="Sample size",ylab="Percentage of explained variance (%)",
     cex.lab=1.5,cex.axis=1.5,ylim=c(-10,110),log="x",col="red",xaxt="n")
lines(seq(n/100,n,by=n/100),S2*100,lwd=2,col="green")
lines(seq(n/100,n,by=n/100),S3*100,lwd=2,col="blue")
lines(seq(n/100,n,by=n/100),S1_up*100,lwd=1,lty=3,col="red")
lines(seq(n/100,n,by=n/100),S2_up*100,lwd=1,lty=3,col="green")
lines(seq(n/100,n,by=n/100),S3_up*100,lwd=1,lty=3,col="blue")
lines(seq(n/100,n,by=n/100),S1_low*100,lwd=1,lty=3,col="red")
lines(seq(n/100,n,by=n/100),S2_low*100,lwd=1,lty=3,col="green")
lines(seq(n/100,n,by=n/100),S3_low*100,lwd=1,lty=3,col="blue")
axis(1, at = c(1000,10000,100000,1000000),cex.axis=1.5)
#abline(h=S_theoretical,lty=2) # theoretical value of S1 and S2
legend(70000,70,lwd=c(2,2,2,2),lty=c(1,1,1,3,2),col = c("red","green","blue","black"),
...
# Record sigma, which can be interpreted as a first-order influence.
r<-100
for (i in 1:100){
  ...
  m1[i] <- apply(M$ee, 2, mean)[1]
  m2[i] <- apply(M$ee, 2, mean)[2]
}
...
par(mar=c(4,5.1,1.6,2.1))
plot(seq(r,r*100,by=r),m1,type="l",lwd=2,xlab="Sample size",ylab="Mu",
     cex.lab=2,cex.axis=2,ylim=c(0,0.5),col="red")
lines(seq(r,r*100,by=r),m2,lwd=2,col="green")
...
# To more accurately interpret the Morris method results, we should compare the importance
